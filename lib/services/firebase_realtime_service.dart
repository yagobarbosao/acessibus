...
class FirebaseRealtimeService {
  ...
  Future<bool> iniciarMonitoramentoRealtime({required String idOnibus}) async {
    ...
    try {
      ...
      _realtimeSubscription = _database
          .child(path)
          .onValue
          .listen(
            (DatabaseEvent event) {
              ...
              print(
                'Firebase Realtime: Dados recebidos - Distância: $distanciaNum, Alerta: $alerta, Ônibus: $onibus',
              );
              if (latNum != null && lonNum != null) {
                print(
                  'Firebase Realtime: Coordenadas - Lat: $latNum, Lon: $lonNum',
                );
              }
              if (fonte.isNotEmpty) {
              ...
              final alertaIndicaProximo =
                  alerta.toLowerCase().contains('proximo') ||
                  alerta.toLowerCase().contains('chegando') ||
                  alerta.toLowerCase().contains('próximo');

              // Verifica se o ônibus está próximo baseado na distância ou alerta
              final onibusProximo =
                  (distanciaNum != null && distanciaNum <= distanciaLimite) ||
                  alertaIndicaProximo;
              ...
              if (onibusProximo && onibus.isNotEmpty) {
                ...
                print(
                  'Firebase Realtime: ✅ Ônibus $linha está próximo! Notificando usuário...',
                );
                ...
                if (distanciaNum != null) {
                  print(
                    'Firebase Realtime: ⏳ Ônibus ainda está longe (${distanciaNum.toStringAsFixed(2)}m > ${distanciaLimite}m)',
                  );
                }
